...
import 'package:cmanga/utils/cbz.dart';
import 'package:cmanga/utils/epub.dart';
import 'package:cmanga/utils/io.dart';
import 'package:cmanga/utils/pdf.dart';
...
class _LocalComicsPageState extends State<LocalComicsPage> {
  ...
  void exportComics(
    ...
    final workingDir = await cache.createTemp('comics_export_');
    final cacheDir = FilePath.join(workingDir.path, 'comics');
    final outFile = FilePath.join(workingDir.path, 'comics_export.zip');
    bool canceled = false;
    void checkCanceled() {
      if (canceled) throw StateError('Export canceled');
    }
    ...
    try {
      ...
      for (var comic in comics) {
        ...
        await export(comic, fileName, checkCanceled: checkCanceled);
        checkCanceled();
        current++;
        if (comics.length > 1) {
