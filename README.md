...
- **处理结果跨阅读会话和应用重启复用**：PNG 与完整性元数据保存在应用支持目录，按输入内容、模型和处理参数区分。默认共享缓存池为 **2 GiB，可在“设置 → 阅读”中修改**。启用漫画专属设置后，可为该漫画设置**独立缓存额度**，涵盖它的所有 AI 阶段，不受共享池上限提前淘汰；未设独立额度的漫画继续共用默认池。各池分别按最近使用情况淘汰，独立额度会增加总磁盘占用。“使用共享池”可取消该漫画的独立额度。调整额度不改变图像参数，也不主动重新推理。
- 返回当前章节已完成的页面时，仍有效的磁盘结果直接读取，不重新排队等待后续页面，也不再把缓存读取标成 AI 推理。缓存损坏、参数变化或结果已被淘汰时才需重新处理；手动“重新处理”仍绕过缓存。提高额度不能恢复已被删除的结果。旧版尚存缓存会在命中时迁入漫画归属目录；未完成结果不会冒充成功缓存，不在内存里保留整章全部位图。
...
- **Apple 最低版本为 macOS 13.3 / iOS 16.3**，还需兼容的 Metal 硬件和模型算子。iOS 导入使用系统文件选择器的沙盒副本，再校验并安装 ONNX；iPad 分享提供弹出位置。
...
缓存额度补充验收：缓存、预加载队列和数字设置的 21 项定向回归通过，覆盖独立 LRU、共享额度、缩减/取消额度、旧缓存迁移、完成页在后续页仍处理时直接返回，以及淘汰后的正常重算。Android 实际界面验证共享池 4 GiB、单漫画 8 GiB、非法输入拒绝、重启保留配置和命中缓存、取消独立额度后恢复共享池。

原生 Metal 检查另确认强制重处理会重新推理、随后可再次命中缓存，并拒绝显式 CPU 请求；中断真实推理后，同一进程重建引擎也能恢复 Metal 推理。2600 万像素 PNG 在当时的设备内存余量下通过头部资源预检，越界尺寸被拒绝；这不承诺任意大图、模型或倍率都能完成推理。
